...
std::string mpz_to_text(const mpz_class& number) {
    ...
    mpz_export(buffer.data(), &count, 1, sizeof(char), 0, 0, number.get_mpz_t());
    return std::string(buffer.data(), count);
}

std::string mpz_to_bytes(const mpz_class& number) {
    size_t count;
    mpz_export(nullptr, &count, 1, sizeof(char), 0, 0, number.get_mpz_t());
    std::vector<char> buffer(count);
    mpz_export(buffer.data(), &count, 1, sizeof(char), 0, 0, number.get_mpz_t());
...
string RaShif_RSA_text(string Shif_text, string d_text, string n_text) { // расшифровка текста RSA
    ...
    //string res = mpz_to_text(rashif_chislo);
    return mpz_to_bytes(rashif_chislo);  // Возвращаем сырые байты
    //return res;
}
...
#ifdef _WIN32
    // Настройка консоли для UTF-8 (для обычного cout)
   /* SetConsoleOutputCP(CP_UTF8);
    SetConsoleCP(CP_UTF8);

    // Для wcout (но лучше использовать WriteConsoleW)
    std::locale::global(std::locale(""));
    std::wcout.imbue(std::locale());
    (setlocale(LC_ALL, "Russian"));*/
     setlocale(LC_ALL, "");
#endif
    /*// Настройка консоли для поддержки UTF-8 (для обычного cout)
    SetConsoleOutputCP(CP_UTF8);
    SetConsoleCP(CP_UTF8);

    // Установка русской локали (для корректной работы функций C++)
    ...
    /*// Ваш код шифрования/дешифрования...
    std::string original_text = "Привет мир!!!! ZOV 123456789";
    std::string e_text = "65537";
    std::string n_text = "";
    std::string d_text = "";

    std::string Shif = Shif_RSA_text(original_text, e_text, n_text);
    std::cout << "Зашифрованный текст: " << Shif << "\n";

    std::string Rashif = RaShif_RSA_text(Shif, d_text, n_text);
    cout << Rashif << endl;
    // Выводим байты в hex-формате для передачи в Qt
    for (char c : Rashif) {
        printf("%02x ", (unsigned char)c);
    }*/
    // Вывод через WriteConsoleW (гарантированно работает)
    //printToConsole(L"Расшифрованный текст: ");
   // printToConsole(Rashif);
   // printToConsole(L"\n");

    // Альтернативный вывод через wcout (может не работать в некоторых случаях)
    // std::wcout << L"Расшифрованный текст: " << Rashif << std::endl;
    ...
    if (argc == 1) {
       ...
       // wcout <<L"ПРивет" << endl;
        //cout << Rashif;
        //cout << "\n";
        #ifdef _WIN32 // выводим в виде байтов, чтобы в Qt считать и перевести в текст это из-за особенностей винды чтобы отображалась кириллица
        for (char c : Rashif) {
            printf("%02x ", (unsigned char)c);
        }
        #else // в линукс проблем чтения из консоли символов с кириллицей нет
        cout << Rashif;
        cout << "\n";
        #endif
        //printToConsole(Rashif);
        //printInConsole(Rashif);
        return 0;
    }
    else {
